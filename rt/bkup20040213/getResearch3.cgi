...
sub getDoc {
    ...
    if ($FORM{type} eq "Research"  && !$FORM{'mailing'} && !$FORM{'focal'} && !$FORM{'fp'}) {
...
EndofHTML
     ...
     } elsif ($FORM{type} eq "FirstTake" && !$FORM{'mailing'}  && !$FORM{'focal'} && !$FORM{'fp'} && !$FORM{'exp'}) {
