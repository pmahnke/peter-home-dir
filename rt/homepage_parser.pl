...
########################################################################
# variables
$URL = "http:\/\/www.gartner.com\/UnrecognizedUserHomePage.jsp";
...
EOF
    ...
    if (/END PRESS RELEASE BLOCK/) {

	$flagPressRel = 0;

	$line =~ s/table width\=\"200\"/table width\=\"186\"/;
	$line =~ s/<td rowspan\=\"100\" bgcolor\=\"\#FFFFFF\" width\=\"100\"\><img src\=\"\/images\/trans_pixel.gif\" width\=\"7\" height\=\"1\" alt\=\"\" border\=\"0\"\><\/td\>//;
	$line =~ s/<td rowspan\=\"100\" bgcolor\=\"\#FFFFFF\" width\=\"7\"\><img src\=\"\/images\/trans_pixel.gif\" width\=\"7\" height\=\"1\" alt\=\"\" border\=\"0\"\><\/td\>//;
	...
	# END of 'IN THE NEWS'
	if (/END FIRST TAKES ASSET/) {

		$flagInTheNews = 0;
		#&write('emea/home_news_headlines.incl', $line);
		#&FTPfile('home_news_headlines.incl');
	...
	# START OF 'IN THE NEWS'
	if (/BEGIN FIRST TAKES ASSET/) {
	...
	# END of FEATURED RESEARCH
	if (/END FEATURED RESEARCH ASSET/) {

		$flagFeaturedResearch = 0;
		#&write('emea/home_fr_headlines.incl', $line);
		#&FTPfile('home_fr_headlines.incl');

	}

    # IN FEATURED RESEARCH
    if ($flagFeaturedResearch) {

        $line .= $_;

    }

	# START OF FEATURED RESEARCH
	if (/BEGIN FEATURED RESEARCH ASSET/) {

		$flagFeaturedResearch = 1;
		undef $line;
    ...
    if (/BEGIN FELLOWS BLOCK/) { #END VENDOR RATING BLOCK/) { #BEGIN WEBLOG BLOCK/) {

	&transVR('home_vr_headlines.incl');
	$flagVRhead = 0;
	undef $line;
	$flagFellows = 1;
	# $flagWeblog = 1; # currently no weblogs
    ...
    # END OF FELLOWS
    if (/END FELLOWS BLOCK/) { # END fellows htm/) {

	$flagFellows = 0;
	&write('emea/home_fellows.incl', $line);
	&FTPfile('home_fellows.incl');
...
ENDofHTML
    ...
    # END OF WEBCASTS
    if (/END WEBCAST ASSET/) {

	$flagWebcast = 0;
	$line = <<ENDofHTML;


<script>function fnCenteredWin(newURL,scrWidth,scrHeight,winFeatures,windowname) {var scrLeft = (window.screen.width/2) - ((scrWidth/2) + 10); var scrTop = (window.screen.height/2) - ((scrHeight/2) + 27);windowparams = "width=" + scrWidth + ",height=" + scrHeight + ",top=" + scrTop + ",left=" + scrLeft + "," + winFeatures ;newWin = window.open(newURL, windowname, windowparams);newWin.focus();return }</script>
<table width="202" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td rowspan="5" bgcolor="#FFFFFF" width="2"><img src="/images/trans_pixel.gif" width="2" height="1" alt="" border="0"></td>
    <td bgcolor="#FFFFFF" width="198" height="2"><img src="/images/trans_pixel.gif" width="198" height="2" alt="" border="0"></td>
    <td rowspan="5" bgcolor="#FFFFFF" width="2"><img src="/images/trans_pixel.gif" width="2" height="1" alt="" border="0"></td>
  </tr>
  ...
  <tr>
    <td valign="top">
      ...
      <table width="198" border="0" cellpadding="0" cellspacing="0">
        <tr>
        <td width="188" height="40" bgcolor="#ADB5C6" colspan="3" valign="center">
          <div><img src="/images/trans_pixel.gif" border="0" width="1" height="2"></div>
          &nbsp;<a href="/research/focus_areas/fa_webcasts/asset_111627_2175.jsp" class="mainNavLink"><b>Sponsored Webcasts<br>&nbsp;Featuring Gartner Analysts</b></a>
          <div><img src="/images/trans_pixel.gif" border="0" width="1" height="2"></div>
          &nbsp;<a href="/research/focus_areas/fa_webcasts/webcast_fa_popup.jsp" onClick="javascript:fnCenteredWin('/research/focus_areas/fa_webcasts/webcast_fa_popup.jsp',402,480,'statusbar','LearnMoreWC'); return false;" class="mainRightNavLink">Learn More</a>
          <div><img src="/images/trans_pixel.gif" border="0" width="1" height="2"></div>
        </td>
      </tr>
...
ENDofHTML
	...
	&write('emea/home_webcasts.incl', $line);
	&FTPfile('home_webcasts.incl');
	&transWebcasts('home_webcasts.incl');
    ...
    # START OF WEBCASTS
    if (/START WEBCAST ASSET/ || $flagWebcast == 1) {
...
sub transPressRel {
    ...
    # translate the Press Release Section
	...
	s/Gartner Press Releases/Comunicati Stampa Gartner/;
	s/More Press Releases//;
	...
	s/Gartner Press Releases/Gartner Pressemitteilungen/;
	s/More Press Releases/Weitere Presseberichte<br \/\>&uuml;ber Gartner/;
...
sub transMQ {
	...
	#chop();
    	...
    	s/Find out which vendors are leaders, visionaries, challengers or niche players./Scopri quali fornitori sono leader, \"visionari\", \"sfidanti\" <br \/\>e \"di nicchia\"/;
...
sub transHC {
	...
	#chop();
    	...
    	s/Gartner Hype Cycles clearly explain the difference between hype and the future of technology./Gartner Hype Cycles zeigen Unterschiede zwischen dem Hype und der Zukunft von Technologien auf/;
	s/View All Hype Cycles/Alle Hype Cycles/;
	s/Latest Hype Cycle:\&nbsp\;<\/b\>/Aktuellster Hype Cycle:\&nbsp\;<\/b\>/;
	s/View the Special Report/Special Report: Gartner Hype Cycles/;
...
EndofHTML
	...
	s/Gartner\'s latest ratings of vendors and <br\>technology providers./Le ultime valutazioni Gartner sui principali fornitori/;
	...
	s/Gartner\'s latest ratings of vendors and <br\>technology providers./Gartners neueste Bewertungen der f\&uuml\;hrenden Anbieter/;
...
sub transResCol {
	...
	#chop();
    	...
    	s/Research Collections/Research Kollektionen/;
        s/Get research articles, tools and other resources./Research, Tools und andere Ressourcen./;
        #s/View All/Weiterer/;
        s/View All Research Collections/Weitere Research Kollektionen/;
...
sub transExecRpt {
    ...
    foreach (@lines) {
        ...
        s/Eight comprehensive reports on key topics including business intelligence, mobile and wireless, CRM and more .../Otto report esaustivi su argomenti chiave comprendenti business intelligence, mobile e wireless, CRM e altro ancora.../;
        s/View All Strategic Planning Reports/Visualizza tutti gli Strategic Planning Reports/;
    ...
    foreach (@lines) {
        ...
        s/Eight comprehensive reports on key topics including business intelligence, mobile and wireless, CRM and more .../Acht ausf\&uuml\;hrliche Berichte \&uuml\;ber Schl\&uuml\;sselthemen wie Business Intelligence, Mobile \&amp\; Wireless <br>und CRM./;
        s/View All Strategic Planning Reports/Alle Strategic Planning Reports/;
...
sub transWebcasts {
 ...
 # s/\>\&nbsp\;Sponsored Webcasts/ style\=\"font-size: 60\%\"\>\&nbsp\;Webcast Sponsorizzati/;
	s/Sponsored Webcasts/Webcast Sponsorizzati/;
	s/Learn More/Ulteriori informazioni/;
	s/More Webcasts/Webcast precedenti/;
	s/Other Webcasts/Altri webcast/;
    ...
    foreach (@lines) {
        ...
        # s/\>\&nbsp\;SPONSORED WEBCASTS/ style\=\"font-size: 60\%\"\>\&nbsp\;GESPONSORTE WEBCASTS/;
	s/Sponsored Webcasts/Sponsoren-Webcasts/;
	s/Featuring Gartner Analysts/mit Gartner Analysten/;
	s/Learn More/Weitere Informationen/;
	s/More Webcasts/Zum Archiv/;
	s/Previous Webcasts/Vorherige Webcasts/;
	s/Spotlight Webcast/Aktueller Webcast/;
	s/Other Webcasts/Weitere Webcasts/;
