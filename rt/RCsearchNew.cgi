...
sub parsePage {
	...
	if (!$pageOneFlag && /Search\?/) {
		...
		$msgT .= "FOUND ADDITIONAL RESULTS PAGE $1<p\>\n\n";
	    }
...
#	$summary{$title} = $1 if (/<span class\=\"resultSummary\"\>(.*[^<\/span\>])<\/span\>/);
	...
	if (/javascript:openDoc\(\'(Display.*[^\&ref\=])\&ref\=/) {
	    $searchURL = $1;
	    $searchURL{$title} = $searchURL if (!$searchURL{$title});
	    $msgT .= "$searchURL \n\t\t$_\n" if (!$searchURL{$title});	
	    # GET DOC DETAILS
...
sub printPage {
	...
	$listing .= "$title\t$summary{$title}\t$author{$title}\t$date{$title}\t$noteNumber{$title}\t$URL{$title}\n";
