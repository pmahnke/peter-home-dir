...
sub getLoggedinResearchDetail {
    ...
    # action
    local @docDetails = &parseLoggedinResearchDetailPage(&getGARTNERpage($_[0]));
    return (@docDetails);

}

################################################
sub parseLoggedinResearchDetailPage {
...
#		print "\n\nFOUND MULTI LINE AUTHOR LINK $1\n\n";
    ...
    # clean up toc
    # removed at we don't need this twice right now ---- but if this was for anything other and DR$toc = &cleanToc($toc);
